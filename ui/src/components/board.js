...
import { gameSlice, movePiece, doHumMove } from '../store/gameSlice';
import './board.css';
...
const Board = () => {
  ...
  const { board, currentPlayer, history, sessionId } = useSelector((state) => state.game);

  const handleClick = (i, j) => {
    console.log(i, j)
    if (board[i][j] == 0) {
      const location = board_size * i + j;
      dispatch(gameSlice.actions.doHumMove(location));
      dispatch(movePiece({ sessionId, location }));
    }
  };
